...
open class InfoLabel: UILabel {
    ...
    public let fontt = UIFont(name: "SFProDisplay-MediumItalic", size: 28.0)
  ...
  fileprivate func updateText(_ string: String) {
    let textAttributes = LightboxConfig.InfoLabel.textAttributes
     // let fontAttribute = [NSAttributedString.Key.font: UIFont(name: "SFProDisplay-MediumItalic", size: 28.0)!]
    ...
    if let range = string.range(of: ellipsis) {
        ...
        //attributedString.addAttributes(fontAttribute, range: ellipsisRange)
    }
  ...
  fileprivate func heightForString(_ string: String) -> CGFloat {
    return string.boundingRect(
      ...
      attributes: [NSAttributedString.Key.font: fontt!],
      context: nil).height
  }

  fileprivate func numberOfLines(_ string: String) -> Int {
      let lineHeight = "A".size(withAttributes: [NSAttributedString.Key.font: fontt!]).height

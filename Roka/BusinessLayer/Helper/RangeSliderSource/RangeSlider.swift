...
open class RangeSlider: UIControl {
    ...
    @IBInspectable open var singleRangeSlider: Bool = false {
        didSet {
            updateLayerFrames()
        }
    }

    @IBInspectable open var multipleRangeSlider: Bool = false {
        didSet {
            updateLayerFrames()
        }
    }
    ...
    open override func continueTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        ...
        if minValueThumbLayer.isHighlight {
            if index > rangeValues.firstIndex(of: maxValue) ?? 0 {
                minValue = maxValue
            } else {
                minValue = rangeValues[index]
            }
        } else if maxValueThumbLayer.isHighlight {
            if index < rangeValues.firstIndex(of: minValue) ?? 0 {
                maxValue = minValue
            } else {
    ...
    open func setMinAndMaxValue(_ minValue: Int, maxValue: Int) {
        if singleRangeSlider || multipleRangeSlider{
            self.minRange = minValue
            self.maxRange = maxValue
        } else {
    ...
    open func setMinMaxValueDisplayTextGetter(_ maxGetter: MaxValueDisplayTextGetter?, _ minGetter: MinValueDisplayTextGetter?) {
        self.maxValueDisplayTextGetter = maxGetter
        self.minValueDisplayTextGetter = minGetter
    }
    ...
    func updateLayerFrames() {
        ...
        if singleRangeSlider{
            minValueThumbLayer.frame = CGRect(x: 0,
                                              y: offsetY,
                                              width: 0,
                                              height: 0)
            ...
            if labelsAreBelow {
                ...
                minValueDisplayLayer.frame = CGRect(x: 0,
                                                    y: offsetY,
                                                    width: 0,
                                                    height: 0)
            }
    ...
    func position(_ value: Int) -> CGFloat {
        if let index = rangeValues.firstIndex(of: value) {
            let count = rangeValues.count
            if index == 0 {
                return thumbRadius
            } else if index == count - 1 {

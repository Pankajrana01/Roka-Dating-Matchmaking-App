...
open class DTPagerController: UIViewController, UIScrollViewDelegate {
    ...
    open var preferredSegmentedControlHeight: CGFloat = 50 {
        didSet {
            view.setNeedsLayout()
        }
    }
    ...
    open var perferredScrollIndicatorHeight: CGFloat = 2 {
        didSet {
            ...
            scrollIndicator.frame.origin.y = segmentedControlHeight - scrollIndicatorHeight + 20
        }
    }
    ...
    open var showBorderLineOnTop: Bool = false
    ...
    public var textColor: UIColor = UIColor.appTextGreyColor {
        didSet {
            if viewIfLoaded != nil {
    ...
    public var selectedTextColor: UIColor = UIColor.appBorder {
        didSet {
            if viewIfLoaded != nil {
    ...
    /// Normal text color in segmented control bar
    /// Default value is UIColor.lightGray
    public var font: UIFont = UIFont(name: "SharpSansTRIAL-Semibold", size: 14.0)! {
        didSet {
            if viewIfLoaded != nil {
                updateSegmentedNormalTitleTextAttributes()
            }
    ...
    /// Selected text color in segmented control bar
    /// Default value is UIColor.black
    public var selectedFont: UIFont = UIFont(name: "SharpSansTRIAL-Semibold", size: 14.0)! {
        didSet {
            if viewIfLoaded != nil {
                updateSegmentedSelectedTitleTextAttributes()
            }
    ...
    override open func viewDidLoad() {
        ...
        // Add subviews
        let label = UILabel.init(frame: CGRect(x: 0, y: 0, width: view.bounds.width, height: 20))
        if GlobalVariables.shared.selectedProfileMode == "MatchMaking" {
            label.backgroundColor = UIColor.loginBlueColor
        } else {
            label.backgroundColor = UIColor.appTitleBlueColor
        }
    ...
    open override func viewDidLayoutSubviews() {
        ...
        pageSegmentedControl.frame = CGRect(x: 0, y: 20, width: view.bounds.width, height: segmentedControlHeight)

        pageSegmentedControl.backgroundColor = .white
        
        if showBorderLineOnTop {
            let label = UILabel()
            label.frame = CGRect(x: pageSegmentedControl.frame.origin.x, y: 20, width: pageSegmentedControl.frame.size.width, height: 1)
            label.backgroundColor = UIColor.appSeparator
    ...
    func setUpPageScrollView() {
        ...
        pageScrollView.frame = CGRect(x: 0, y: segmentedControlHeight+20, width: size.width, height: size.height - segmentedControlHeight)
    ...
    open func setUpScrollIndicator() {
        if numberOfPages > 0 {
            scrollIndicator.frame.size = CGSize(width: view.bounds.width/CGFloat(numberOfPages) - 30, height: scrollIndicatorHeight)
        }

        scrollIndicator.frame.origin.y = segmentedControlHeight - scrollIndicatorHeight + 21
